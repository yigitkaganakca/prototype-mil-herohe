#!/usr/bin/env bash
# Valieris 3-class (neg/low/high) — matched protocol: folds_phiher2_binary_s42 + val_loss + test ensemble.
# Models: ABMIL, CLAM, TransMIL, khead L8 (binary-tuned reg recipe).
#
# Usage:
#   run_threeclass_5fold_s42.sh abmil|clam|transmil|khead|all [fold|all]
#   run_threeclass_5fold_s42.sh eval
set -euo pipefail
...
KHEAD_TEST_OUT="$REPO/herohe/gp2/runs/test_eval_phenonly/khead_valieris3_tuned"
LOG="$REPO/herohe/gp2/data/threeclass_5fold_s42.log"
L=8
SEED="${SEED:-0}"
FORCE="${FORCE:-0}"

# khead tuned reg (binary winner)
K_DROPOUT=0.6
K_WD=0.004
K_ENT=0.15
K_PD=0.20
...
run_dir_for() {
  case "$1" in
    abmil) echo "$REPO/herohe/gp2/runs/abmil_valieris3_5fold_s42_valloss" ;;
    clam) echo "$REPO/herohe/gp2/runs/clam_valieris3_5fold_s42_valloss" ;;
    transmil) echo "$REPO/herohe/gp2/runs/transmil_valieris3_5fold_s42_valloss" ;;
    khead) echo "$REPO/herohe/gp2/runs/khead_valieris3_tuned_5fold_s42" ;;
...
train_baseline_one() {
  ...
  echo "========== $(date) 3-class $A fold-$F START =========="
...
train_khead_one() {
  local F="$1"
  local RUN="$REPO/herohe/gp2/runs/khead_valieris3_tuned_5fold_s42"
  local CKPT="$RUN/fold_${F}/best.pt"
  if [[ -f "$CKPT" && "$FORCE" != "1" ]]; then
    echo "[skip] khead 3-class fold $F"
    return 0
  fi
  if [[ "$FORCE" == "1" && -d "$RUN/fold_${F}" ]]; then
    mv "$RUN/fold_${F}" "$RUN/fold_${F}_prev_$(date +%Y%m%d_%H%M%S)"
  ...
  echo "========== $(date) 3-class khead fold-$F START =========="
...
eval_baseline() {
  ...
  local RUN TAG
  RUN="$(run_dir_for "$A")"
  TAG="${A}_valieris3_5fold_s42_valloss_5fold"
  local CKPTS=()
...
eval_khead() {
  ...
  "$PY" "$REPO/herohe/gp2/scripts/eval_phenobin_test.py" \
    ...
    --device mps \
    --max_patches -1 \
    --tag "khead_valieris3_tuned_5fold" \
...
exec > >(tee -a "$LOG") 2>&1
echo "========== $(date) threeclass_5fold_s42 agg=$AGG fold=$FOLD =========="
...
echo "========== $(date) threeclass_5fold_s42 DONE =========="
