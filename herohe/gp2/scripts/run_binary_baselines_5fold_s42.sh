#!/usr/bin/env bash
# Binary HEROHE baselines — SAME protocol as khead_phiher2fold_abmil_recipe:
#   5-fold stratified CV (folds_phiher2_binary_s42.csv) + val_loss + 5-fold test ensemble.
#   NOT PhiHER2 S2 holdout (that was a separate exploratory run).
#
# Usage:
#   run_binary_baselines_5fold_s42.sh abmil [fold|all]
#   run_binary_baselines_5fold_s42.sh clam [fold|all]
#   run_binary_baselines_5fold_s42.sh transmil [fold|all]
#   run_binary_baselines_5fold_s42.sh all
#   run_binary_baselines_5fold_s42.sh eval
set -euo pipefail
...
# Canonical run dirs (5-fold CV s42). Also accepts legacy *_phiher2fold_valloss from early misnamed run.
run_dir_for() {
  local A="$1"
...
extra_args_for() {
  case "$1" in
    abmil)
      echo --abmil_hidden 512 --abmil_attn 256 --abmil_dropout 0.5 --w_attn_entropy 0.05
      ;;
    clam)
      echo --clam_dropout 0.5 --k_sample 8 --bag_weight 0.7
      ;;
    transmil)
      echo --trans_d_model 512 --trans_layers 2 --trans_heads 8 --trans_dropout 0.25
      ;;
  esac
}
...
train_one() {
  ...
  if [[ -f "$CKPT" && "$FORCE" != "1" ]]; then
    echo "[skip] $A fold $F → $CKPT"
    return 0
  fi

  if [[ "$FORCE" == "1" && -d "$RUN/fold_${F}" ]]; then
    local BAK="$RUN/fold_${F}_prev_$(date +%Y%m%d_%H%M%S)"
    echo "[retry] $A fold $F → $BAK"
    mv "$RUN/fold_${F}" "$BAK"
  fi

  echo "========== $(date) $A fold-$F | 5-fold CV s42 + val_loss START =========="
  echo "Folds: $FOLDS (same as khead_phiher2fold_abmil_recipe)"
...
eval_agg() {
  ...
  local RUN TAG
  RUN="$(run_dir_for "$A")"
  TAG="${A}_binary_5fold_s42_valloss_5fold"
  local CKPTS=()
  for f in 0 1 2 3 4; do
    CKPTS+=("$RUN/fold_${f}/best.pt")
  done
  for c in "${CKPTS[@]}"; do
    if [[ ! -f "$c" ]]; then
...
echo "========== $(date) binary baselines 5-fold-s42 agg=$AGG fold=$FOLD =========="
...
test_out = repo / "herohe/gp2/runs/test_eval_mil/binary_5fold_s42"
khead = repo / "herohe/gp2/runs/test_eval_phenonly/khead_phiher2fold_abmil_recipe"
rows = []
for name, tag in [
    ("khead", "khead_phiher2fold_abmil_recipe_5fold_retry4"),
...
echo "========== $(date) binary baselines DONE =========="
