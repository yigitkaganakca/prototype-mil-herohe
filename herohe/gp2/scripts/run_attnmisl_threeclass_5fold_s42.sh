#!/usr/bin/env bash
# AttnMISL Valieris 3-class — paper-faithful MI-FCN + phenotype-level attention (Yao MedIA 2020).
# Same protocol as run_threeclass_5fold_s42.sh; fold-wise AP prototypes (L=8).
#
# Usage:
#   run_attnmisl_threeclass_5fold_s42.sh [fold|all]
#   run_attnmisl_threeclass_5fold_s42.sh eval
set -euo pipefail
...
RUN="$REPO/herohe/gp2/runs/attnmisl_paper_valieris3_5fold_s42_valloss"
TEST_OUT="$REPO/herohe/gp2/runs/test_eval_mil/valieris3_5fold_s42"
LOG="$REPO/herohe/gp2/data/attnmisl_paper_threeclass_5fold_s42.log"
L=8
SEED="${SEED:-0}"
FORCE="${FORCE:-0}"
...
train_one() {
  ...
  if [[ -f "$CKPT" && "$FORCE" != "1" ]]; then
    echo "[skip] attnmisl 3-class fold $F"
    return 0
  fi
  ...
  echo "========== $(date) attnmisl 3-class fold-$F START proto=$PROTO =========="
...
eval_all() {
  local TAG="attnmisl_paper_valieris3_5fold_s42_valloss_5fold"
  local CKPTS=()
...
exec > >(tee -a "$LOG") 2>&1
echo "========== $(date) attnmisl 3-class 5-fold-s42 fold=$FOLD =========="
...
echo "========== $(date) attnmisl 3-class DONE =========="
