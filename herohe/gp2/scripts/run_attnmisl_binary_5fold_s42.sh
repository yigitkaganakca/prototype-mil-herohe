#!/usr/bin/env bash
# AttnMISL binary baseline — paper-faithful MI-FCN + phenotype-level attention (Yao MedIA 2020).
# Same HEROHE protocol as run_binary_baselines_5fold_s42.sh; fold-wise AP prototypes (L=8).
#
# Usage:
#   run_attnmisl_binary_5fold_s42.sh [fold|all]
#   run_attnmisl_binary_5fold_s42.sh eval
set -euo pipefail
...
RUN="$REPO/herohe/gp2/runs/attnmisl_paper_binary_5fold_s42_valloss"
TEST_OUT="$REPO/herohe/gp2/runs/test_eval_mil/binary_5fold_s42"
LOG="$REPO/herohe/gp2/data/attnmisl_paper_binary_5fold_s42.log"
L=8
SEED="${SEED:-0}"
FORCE="${FORCE:-0}"
...
train_one() {
  ...
  echo "========== $(date) attnmisl fold-$F START proto=$PROTO =========="
...
eval_all() {
  local TAG="attnmisl_paper_binary_5fold_s42_valloss_5fold"
  local CKPTS=()
...
exec > >(tee -a "$LOG") 2>&1
echo "========== $(date) attnmisl binary 5-fold-s42 fold=$FOLD =========="
...
echo "========== $(date) attnmisl binary DONE =========="
