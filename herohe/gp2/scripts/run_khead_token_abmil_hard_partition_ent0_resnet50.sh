#!/usr/bin/env bash
# Binary token_abmil + hard_partition routing on ResNet-50 bags (1024-D).
# Same ls10 ent0 recipe as Virchow2 primary model.
#
# Usage:
#   run_khead_token_abmil_hard_partition_ent0_resnet50.sh [fold|all|eval]
set -euo pipefail
...
RUN="$REPO/herohe/gp2/runs/khead_token_abmil_hard_partition_ent0_resnet50"
LOG="$REPO/herohe/gp2/data/khead_token_abmil_hard_partition_ent0_resnet50.log"
L=8
SEED="${SEED:-0}"
FORCE="${FORCE:-0}"
...
train_one() {
  ...
  if [[ -f "$CKPT" && "$FORCE" != "1" ]]; then
    echo "[skip] hard_partition ent0 resnet50 binary fold $F → $CKPT"
    return 0
  fi
  ...
  echo "========== $(date) hard_partition ent0 resnet50 binary fold-$F START =========="
  "$PY" "$REPO/herohe/gp2/scripts/train_phenobin_mil.py" \
    ...
    --K "$L" --num_classes 2 --label_mode gt_binary --feature_dim 1024 \
...
eval_all() {
  local TAG="khead_token_abmil_hard_partition_ent0_resnet50_5fold"
  local CKPTS=()
...
exec >> "$LOG" 2>&1
echo "========== $(date) hard_partition ent0 resnet50 binary fold=$FOLD =========="
...
echo "========== $(date) hard_partition ent0 resnet50 binary DONE =========="
